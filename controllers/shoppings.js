import shoppings from '../models/shoppings.js'
import users from '../models/users.js'

export const createShoppings = async (req, res) => {
  try {
    // 檢查購物車是不是空的
    ...
    // 建立訂單
    result = await shoppings.create({ u_id: req.user._id, name: req.user.name, products: req.user.cart })
    // 清空購物車
    req.user.cart = []
    await req.user.save()
...
export const getAllShoppings = async (req, res) => {
  try {
    ...
    const result = await shoppings.find().populate('products.p_id').populate('u_id', 'account')
    res.status(200).json({ success: true, message: '', result })
  } catch (error) {
    res.status(500).json({ success: false, message: '未知錯誤' })
  }
